...
public class CanalUtils {
     ...
     * @return   GTID
     */
    public static String getCurrentGtid(DebeziumEntry.Header header) {
        List<DebeziumEntry.Pair> props = header.getPropsList();
        if (props != null && props.size() > 0) {
            for (DebeziumEntry.Pair pair : props) {
     ...
     */
    public static String getCurrentGtidSn(DebeziumEntry.Header header) {
        List<DebeziumEntry.Pair> props = header.getPropsList();
        if (props != null && props.size() > 0) {
            for (DebeziumEntry.Pair pair : props) {
     ...
     */
    public static String getCurrentGtidLct(DebeziumEntry.Header header) {
        List<DebeziumEntry.Pair> props = header.getPropsList();
        if (props != null && props.size() > 0) {
            for (DebeziumEntry.Pair pair : props) {
